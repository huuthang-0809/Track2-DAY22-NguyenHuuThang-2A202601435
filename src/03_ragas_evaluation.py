...
# Compatibility patch cho Ragas với LangChain 1.x / 0.4
import types
if "langchain_community.chat_models.vertexai" not in sys.modules:
    try:
        import langchain_google_vertexai
        _mod = types.ModuleType("langchain_community.chat_models.vertexai")
        _mod.ChatVertexAI = langchain_google_vertexai.ChatVertexAI
...
SYSTEM_V2 = (
    ...
    "2) Trích dẫn thông tin từ context, "
    "3) Nêu rõ độ tin cậy. "
    "Luôn dựa vào dữ liệu được cung cấp, không suy đoán thêm.\n\nContext:\n{context}"
)
    ...
    for attempt in range(5):
        try:
            ...
            if ("429" in str(e) or "RESOURCE_EXHAUSTED" in str(e)) and attempt < 4:
                print(f" ⏳ Rate Limit, tạm dừng 10s...")
                time.sleep(10)
            else:
                raise e
...
def run_ragas_eval(rag_results: list, version: str) -> dict:
    ...
    for key in ["faithfulness", "answer_relevancy", "context_recall", "context_precision"]:
        ...
        if isinstance(raw, (list, np.ndarray)):
            clean_vals = [v for v in raw if v is not None and not np.isnan(v)]
            scores[key] = float(np.mean(clean_vals)) if clean_vals else 0.0
